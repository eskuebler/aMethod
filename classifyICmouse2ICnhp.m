...
count = 1;
for n = 1:size(YTest2.ICnhp.NN,1)
    for ct = 1:2
        ctConf.ICnhp.nn(count,ct) = sum(YTest2.ICnhp.NN(n,:)==ct)/size(YTest2.ICnhp.NN,2);
    end
...
for n = 1:length(predCT(:,1))
    if predCT(n,1) == 1; temp(n,:) = 1; C(n,:) = plots.cc(1,:);
    elseif predCT(n,1) == 2; temp(n,:) = 1; C(n,:) = plots.cc(1,:);
    elseif predCT(n,1) == 3; temp(n,:) = 1; C(n,:) = plots.cc(1,:);
    elseif predCT(n,1) == 4; temp(n,:) = 2; C(n,:) = plots.cc(4,:);
    end
end
figure('Position',[50 50 270 200]); set(gcf,'color','w');
    scatter3(Xnhp(:,1),Xnhp(:,2),Xnhp(:,3),...
...
for n = 1:length(predCT(:,1))
    if predCT(n,1) == 1; C(n,:) = plots.cc(1,:);
    elseif predCT(n,1) == 2; C(n,:) = plots.cc(4,:);
    end
end    
figure('Position',[50 50 270 200]); set(gcf,'color','w');
    scatter3(Xnhp(:,1),Xnhp(:,2),Xnhp(:,3),...
